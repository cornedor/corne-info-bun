...
let printPrice = value => {
  let a = Intl.NumberFormat.makeWithLocaleAndOptions(
    "nl",
    {
      "currency": "EUR",
    },
  )
  Intl.NumberFormat.format(a, value)
}

let printDate = date => {
  Intl.DateTimeFormat.makeWithLocaleAndOptions(
    "sv",
    {
      "timeStyle": "short",
    },
...
let make = (~marketData: Js.Nullable.t<marketData>, ~incorrectJson) => {
  ...
  | Some(marketData) => {
      ...
      <div>
        <div className="marketdata font-mono text-sm overflow-y-hidden">
          {React.array(prices)}
        </div>
      </div>
    }
